...
- Use optional proot launch profiles for normal, isolated, or minimal sessions.
...
catainer shell u24
catainer shell u24 --profile isolated
```
...
```text
catainer shell NAME [--user USER] [--workdir DIR] [--host-root] [--profile normal|isolated|minimal] [-- COMMAND...]
catainer exec NAME [--user USER] [--workdir DIR] [--host-root] [--profile normal|isolated|minimal] -- COMMAND...
catainer login NAME [--user USER] [--workdir DIR] [--host-root] [--method auto|runuser|su|login] [--profile normal|isolated|minimal]
catainer root-shell NAME [--user USER] [--workdir DIR] [--profile normal|isolated|minimal] [-- COMMAND...]
catainer root-exec NAME [--user USER] [--workdir DIR] [--profile normal|isolated|minimal] -- COMMAND...
```
...
catainer doctor [NAME] [--host-root] [--run-tests] [--profile normal|isolated|minimal] [--print-proot-cmd]
...
catainer --profile isolated shell u24
...
Launch profiles adjust which host paths are bound for a session. They do not edit or delete saved container configuration.

```sh
catainer shell u24 --profile normal
catainer shell u24 --profile isolated
catainer shell u24 --profile minimal
catainer exec u24 --minimal -- uname -a
catainer login u24 --profile isolated
```

Profiles:

```text
normal    current default; applies Android runtime binds and saved mounts
isolated  keeps Android runtime binds, but skips saved mounts for this launch
minimal   binds only /dev, /proc, and /sys, and skips saved mounts
```

Saved extra `proot` arguments still apply in all profiles. Inspect the exact launch command:

```sh
catainer doctor u24 --print-proot-cmd
catainer doctor u24 --profile minimal --print-proot-cmd
```
...
- Added opt-in proot launch profiles: `normal`, `isolated`, and `minimal`.
- Added `catainer doctor NAME --print-proot-cmd` to inspect the generated proot command.
- Added `--profile`, `--normal`, `--isolated`, and `--minimal` for shell, exec, login, root-shell, root-exec, and doctor flows.
