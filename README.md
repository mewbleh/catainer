...
- List and remove mounts, environment variables, and extra `proot` arguments without hand-editing files.
- Check and repair missing container CA bundles for `curl`, `git`, and package managers.
- Use optional host-root mode on rooted devices through `su` or `tsu`.
- Generate Bash or Zsh shell completion.
...
- Back up, restore, clone, rename, upgrade, inspect, and delete containers from one CLI.
...
Repair container CA certificates if HTTPS tools report certificate bundle errors:
...
If `curl`, `git`, or an installer fails inside a container with a missing certificate bundle such as `curl: (77) error setting certificate file: /etc/ssl/certs/ca-certificates.crt`, repair the container CA package from the Termux side:
...
`clone` copies the container directory and refreshes the new container metadata.
`rename` moves the container directory and refreshes the name, hostname, prompt metadata, and guest info files.
...
- Kept the experimental PAM/session login path as the default for `catainer login`.
